...
public record PaleMossTreeDecorator(float groundChance, float leavesChance, float trunkChance) implements TreeDecorator {
	public static final MapCodec<PaleMossTreeDecorator> CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
			...
			Codec.FLOAT.fieldOf("ground_probability").forGetter(PaleMossTreeDecorator::groundChance),
			Codec.FLOAT.fieldOf("leaves_probability").forGetter(PaleMossTreeDecorator::leavesChance),
			Codec.FLOAT.fieldOf("trunk_probability").forGetter(PaleMossTreeDecorator::trunkChance)
