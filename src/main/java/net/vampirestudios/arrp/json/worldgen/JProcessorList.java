...
public class JProcessorList {
	public static final Codec<JProcessorList> CODEC = RecordCodecBuilder.create(i -> i.group(
			Processor.CODEC.listOf().fieldOf("processors").forGetter(x -> x.processors)
	).apply(i, processors -> new JProcessorList().processors(processors)));

	private List<Processor> processors = new ArrayList<>();

	public static JProcessorList processorList() { return new JProcessorList(); }
	public JProcessorList processors(List<Processor> processors) { this.processors = new ArrayList<>(processors); return this; }
	public JProcessorList processor(Processor processor) { this.processors.add(processor); return this; }
	public JProcessorList blockIgnore(List<String> blocks) { return processor(Processor.blockIgnore(blocks)); }
	public JProcessorList rule(Rule rule) { return processor(Processor.ruleProcessor(rule)); }
