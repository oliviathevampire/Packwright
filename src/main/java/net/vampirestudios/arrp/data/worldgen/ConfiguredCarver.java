...
public class ConfiguredCarver {
	public static final Codec<ConfiguredCarver> CODEC = RecordCodecBuilder.create(i -> i.group(
			Identifier.CODEC.fieldOf("type").forGetter(x -> x.type),
			Config.CODEC.fieldOf("config").forGetter(x -> x.config)
	).apply(i, (type, config) -> new ConfiguredCarver().type(type).config(config)));

	private Identifier type = Identifier.withDefaultNamespace("cave");
	private Config config = new Config();

	public static ConfiguredCarver carver() {
		return new ConfiguredCarver();
	}
	...
	public ConfiguredCarver type(Identifier type) { this.type = type; return this; }
